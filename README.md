# Evolve

A turn-based card roguelite. Choose **Druid**, **Priest**, or **Shaman** at the start.

## Play

```bash
npm install
npm run dev
```

Open the URL Vite prints (usually `http://localhost:5173`).

## How to play

1. **Choose Druid, Priest, or Shaman** on the title screen.
2. Pick a path node on the map (fights, elites, rest, treasure, shop, boss).
3. In combat: click a card, then click an enemy if it needs a target. **End Turn** when ready.
4. After victory, pick a reward card and spend a **talent point** in your class trees.
5. **Defeat an elite** to choose **1 of 3 items** (general relics + your specialization's unique set).
6. Defeat the **Nightmare of the Grove**, then push into **The Barrens**.
7. Face random **Wailing Caverns** elites and defeat **Mutanus the Devourer**.

### Items

Elites drop relics. The offer pool is **20 general items** (any class) plus **5 unique items per opening specialization**. Spec items only appear for that opening. Keep one of three each elite clear.

### Druid opening specs

| Spec | Style | Signature cards |
|------|--------|-----------------|
| Feral | Claws & hide (Cat + Bear) | Swipe, Maul, Claw, Shred, Rip |
| Boomkin | Big spells | Wrath, Starfire, Starsurge |
| Tree | Healing | Decurse, Rejuvenation, Healing Touch |

Individual cards still carry **Bear** or **Cat** form tags for borders and Feral talent matching.

### Priest schools

| School | Style | Signature cards |
|--------|--------|-----------------|
| Discipline | Shields & atonement | Smite, Penance, Power Word: Shield |
| Shadow | DoTs & void damage | Mind Blast, Shadow Word: Pain, Void Eruption |
| Holy | Direct healing | Flash Heal, Renew, Divine Hymn |

### Shaman paths

| Path | Style | Signature cards |
|------|--------|-----------------|
| Resto | Heals & water totems | Healing Wave, Riptide, Healing Stream |
| Enhance | Melee storms & earth totems | Stormstrike, Windfury, Strength of Earth |
| Elemental | Shocks & fire totems | Lightning Bolt, Flame Shock, Searing Totem |

**Totems** are Shaman's signature mechanic: fragile allies (low HP) that absorb damage before you and grant strong auras while alive. Only one totem per element (Earth / Fire / Water / Air).

### Talent trees

Classic-style trees: spend points in a tree to unlock deeper tiers (3 points per tier). Prerequisites chain key talents. Capstones reward specialization.

**Druid:** Feral · Restoration · Balance  
**Priest:** Holy · Shadow · Discipline  
**Shaman:** Restoration · Enhancement · Elemental

**Spell Power** boosts caster schools (Boomkin / Holy / Shadow / Discipline / Elemental) — never Bear, Cat, or Enhance melee.

Open **Talents** from the map (or after a fight) to allocate points. Hover nodes for details; card text updates to show talent bonuses.

## Stack

- Vite + TypeScript
- Phaser 4
- AI-generated WoW-inspired fantasy art
